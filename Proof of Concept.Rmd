...
We also want a unique database of the tracks in our colocation table, so we can access their information.

```{r track_data}
track_data <- unique(geo_tracks[complete.cases(geo_tracks) & geo_tracks$track_uri %in% track_colocation$track_uri,4:9])
```
...
saveRDS(track_colocation, "data/track_colocation.rds")
saveRDS(track_data, "data/track_data.rds")
```
